...
const dayTarget = { date: '2026-04-13', kind: 'day', viewerTimeZone: 'Asia/Tokyo' } as const;
...
describe('itinerary note drafts', () => {
	...
	it('creates a daily draft at local noon in the supplied default time zone', () => {
	...
	it('anchors a new daily note to its viewer-local schedule day', () => {
		const draft = itineraryNoteDraft(
			undefined,
			'Asia/Tokyo',
			{ date: '2026-04-12', kind: 'day', viewerTimeZone: 'America/Los_Angeles' },
			() => 'day-note-1'
		);

		expect(draft).toMatchObject({
			anchorAt: Date.UTC(2026, 3, 12, 19),
			anchorDate: '2026-04-13',
			anchorTime: '04:00',
			timeZone: 'Asia/Tokyo'
		});
	...
	it('keeps a valid daily anchor fixed while changing its entry-time zone', () => {
		...
		expect(validation.note).toMatchObject({ anchorAt: Date.UTC(2026, 3, 13, 3) });
	});

	it('rejects an anchor time skipped by daylight saving', () => {
		const draft: ItineraryNoteDraft = {
			anchorAt: null,
	...
	it('retains an anchor in the second occurrence of a repeated local hour', () => {
		...
		const draft = itineraryNoteDraft(
			...
			'UTC',
			{ date: '2026-11-01', kind: 'day', viewerTimeZone: 'America/Los_Angeles' }
		);
	...
	it('preserves a changed anchor when it falls in a repeated hour after a time-zone change', () => {
		const draft = itineraryNoteDraft(
			undefined,
			'UTC',
			{ date: '2026-11-01', kind: 'day', viewerTimeZone: 'UTC' },
			() => 'day-note-1'
		);
		if (draft.kind !== 'day') {
			throw new Error('The day target should create a day note draft.');
		}
		draft.anchorTime = '09:30';
		const changedTimeZone = itineraryNoteDraftForTimeZone(draft, 'America/Los_Angeles');

		expect(changedTimeZone).toMatchObject({
			anchorAt: Date.UTC(2026, 10, 1, 9, 30),
		...
		expect(validation.note).toMatchObject({ anchorAt: Date.UTC(2026, 10, 1, 9, 30) });
	});
});
